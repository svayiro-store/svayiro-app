        ...
        <div className="space-y-6">
          {orders.map((ord) => {
            const statusColors: Record<string, string> = {
              ...
              cancelled: 'bg-rose-50 text-red-600 border-rose-200   '
            };
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-100 ">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-mono font-extrabold text-indigo-700 bg-indigo-500/10 px-2.5 py-0.5 rounded-lg ">
                        {ord.id}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {isUpiPendingVerification && (
                      <span className="text-[10px] px-2.5 py-1 rounded-full uppercase font-bold tracking-wider bg-amber-50 text-amber-700 border border-amber-200    animate-pulse flex items-center gap-1">
                ...
                <div className="py-1">
                  <p className="text-[10px] font-semibold text-slate-400  uppercase tracking-widest mb-2">Ordered Items</p>
                  <div className="space-y-1.5">
                    {ord.items.map((item, idx) => (
                      <div key={idx} className="text-xs font-bold text-slate-800  flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-mono text-slate-500  bg-slate-100  px-2 py-0.5 rounded shrink-0 font-bold">
                          Qty: {item.quantity}
                        </span>
                      </div>
                    ))}
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-100 ">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-extrabold text-indigo-600  text-sm">₹{ord.finalTotal}</span>
                  </div>
                ...
                {isExpanded && (
                  <div
                    className="pt-4 border-t border-slate-100  mt-3 space-y-5 animate-fadeIn"
                    onClick={(e) => {
                  ...
                  >
                    ...
                    {!isCancelled ? (
                      ...
                      <div className="p-3 bg-rose-500/5 rounded-xl border border-rose-500/10 text-center font-mono">
                        <p className="text-[11px] text-red-600  font-extrabold flex items-center justify-center gap-1">
                    ...
                    <div className="space-y-3">
                      <p className="text-[10px] font-semibold text-slate-400  uppercase tracking-widest mt-1">Detailed Item Breakdown</p>
                          ...
                          return (
                            ...
                            >
                              <img
                                ...
                                className="h-12 w-12 object-cover rounded-lg shrink-0 border border-slate-200  bg-white shadow-sm"
                              />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 rounded-xl bg-slate-50/50  border border-slate-100  text-xs">
                      <div className="space-y-1 border-b md:border-b-0 md:border-r border-slate-100  pb-3 md:pb-0 md:pr-4">
                        ...
                        <p className="text-slate-500  mt-1">Slot: {ord.selectedSlot}</p>
                        {ord.deliveryAddress && (
                          <p className="text-slate-500  truncate mt-1 text-[11px]" title={`${ord.deliveryAddress.flatAndHouse}, ${ord.deliveryAddress.areaAndStreet}`}>
                            📍 {ord.deliveryAddress.flatAndHouse}, {ord.deliveryAddress.areaAndStreet}
                          </p>
                        )}
                      </div>

                      <div className="space-y-1 border-b md:border-b-0 md:border-r border-slate-100  pb-3 md:pb-0 md:px-4">
                        <p className="text-[10px] font-bold uppercase opacity-60 tracking-wider">Financial summary</p>
                        <p className="font-bold text-lg text-indigo-700  mt-0.5">₹{ord.finalTotal}</p>
                        <div className="text-[11px] opacity-75 font-mono mt-1 flex items-center gap-1.5">
                          <span className="bg-slate-200  px-1.5 py-0.2 rounded text-[10px] uppercase font-bold text-slate-700 ">
                            {ord.paymentDetails?.method || ord.paymentMethod || 'COD'}
                          </span>
                          <span className={`font-semibold uppercase ${(ord.paymentDetails?.status || ord.paymentStatus) === 'paid' ? 'text-emerald-600 ' : 'text-amber-600 '}`}>
                    ...
                    <div className="pt-3 border-t border-slate-100  space-y-3">
                      {orderIdToCancel === ord.id ? (
                        <div className="p-3 bg-rose-50  rounded-xl border border-rose-200  flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                          <div>
                            <p className="text-xs text-rose-700  font-extrabold uppercase tracking-wide">
                              Cancel Order #{ord.id}?
                            </p>
                          ...
                          <div className="flex gap-2 self-end sm:self-auto">
                            ...
                            <button
                              ...
                              disabled={isCancellingApi}
                              className="bg-slate-200 hover:bg-slate-300  text-slate-700  font-bold text-[11px] px-3 py-1.5 rounded-lg transition-all"
                            >
                              Keep Order
                            </button>
                          </div>
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3 select-none">
                          {(ord.status === 'pending' || ord.status === 'accepted' || ord.status === 'packed') ? (
                            <button
                              ...
                              className="bg-rose-50 hover:bg-rose-100  text-rose-600 hover:text-rose-700 font-extrabold text-xs px-4 py-2 border border-rose-200  rounded-full flex items-center gap-1.5 transition-all shadow-sm shrink-0 uppercase tracking-wider font-mono"
                            >
