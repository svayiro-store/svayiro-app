              ...
              <div key={notif.id} className={`rounded-xl border p-4 shadow-sm ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => startEdit(notif)} className="rounded-lg border border-slate-300 p-2 "><Edit3 className="h-3.5 w-3.5" /></button>
                    <button onClick={() => handleDelete(notif.id)} className="rounded-lg border border-rose-300 p-2 text-rose-600  "><Trash2 className="h-3.5 w-3.5" /></button>
                  </div>
                </div>
                <p className="mt-3 text-sm text-slate-700 ">{notif.message}</p>
