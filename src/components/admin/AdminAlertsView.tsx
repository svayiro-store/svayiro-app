  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="font-serif text-3xl font-semibold">Admin Alerts</h1>
        ...
        <button
          type="button"
          onClick={loadAlerts}
          className="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-700 px-4 py-2 text-xs font-semibold text-white shadow-sm"
        >
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <div className={`rounded-xl border p-4 ${panelClass}`}>
          <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">Showing</p>
          <p className="mt-2 text-2xl font-semibold">{counts.total}</p>
        </div>
        <div className={`rounded-xl border p-4 ${panelClass}`}>
          <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">Unread In View</p>
          <p className="mt-2 text-2xl font-semibold text-indigo-500">{counts.unread}</p>
        </div>
        <div className={`rounded-xl border p-4 ${panelClass}`}>
          <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">Critical In View</p>
          <p className="mt-2 text-2xl font-semibold text-rose-500">{counts.critical}</p>
        </div>
      </div>
      ...
      <div className={`rounded-xl border p-4 ${panelClass}`}>
        <div className="grid gap-3 md:grid-cols-[180px_220px_1fr] md:items-end">
          ...
          <label className="block">
            <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">Alert Type</span>
                ...
                <article key={alert.id} className={`border-b p-4 last:border-b-0 ${isDarkMode ? 'border-slate-800' : 'border-slate-100'}`}>
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    ...
                    <div className="flex shrink-0 flex-wrap gap-2">
                      {alert.status !== 'read' && (
                        <button type="button" onClick={() => updateStatus(alert.id, 'read')} className="inline-flex items-center gap-1 rounded-lg bg-emerald-600 px-3 py-2 text-[11px] font-semibold text-white">
                          <CheckCircle2 className="h-3.5 w-3.5" />
                          Read
                        </button>
                      )}
                      {alert.status !== 'archived' && (
                        <button type="button" onClick={() => updateStatus(alert.id, 'archived')} className="inline-flex items-center gap-1 rounded-lg bg-slate-700 px-3 py-2 text-[11px] font-semibold text-white">
                          <Archive className="h-3.5 w-3.5" />
                          Archive
                        </button>
                      )}
                      <button type="button" onClick={() => deleteAlert(alert.id)} className="inline-flex items-center gap-1 rounded-lg bg-rose-600 px-3 py-2 text-[11px] font-semibold text-white">
